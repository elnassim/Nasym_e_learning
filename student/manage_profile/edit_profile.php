...
// Fetch current user's informatio
$studentID = $_SESSION["StudentID"];
$sql = "SELECT FirstName, LastName, Email FROM Students WHERE StudentID = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($firstName_err) && empty($lastName_err) && empty($email_err) && empty($password_err) && empty($confirmPassword_err)) {
        ...
        $sql = "UPDATE Students SET FirstName = ?, LastName = ?, Email = ? WHERE StudentID = ?";
        $stmt = $conn->prepare($sql);
        if ($stmt->execute([$firstName, $lastName, $email, $studentID])) {
