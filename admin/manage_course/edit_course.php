...
if (isset($_GET['id'])) {
    // Get the course ID to edit from the GET request
    $courseID = $_GET['id'];
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $updateCourseSql = "UPDATE Courses SET Title = :title, Description = :description, Keywords = :keywords, ProfessorID = :professor WHERE CourseID = :courseID";
        $updateCourseStmt = $conn->prepare($updateCourseSql);
        $updateCourseStmt->bindParam(':title', $title);
        $updateCourseStmt->bindParam(':description', $description);
        $updateCourseStmt->bindParam(':keywords', $keywords);
        $updateCourseStmt->bindParam(':professor', $professor);
        $updateCourseStmt->bindParam(':courseID', $courseID);
    ...
    // Retrieve professor information from the database
    $getProfessorsSql = "SELECT * FROM professors";
    $getProfessorsStmt = $conn->query($getProfessorsSql);
    $professorsData = $getProfessorsStmt->fetchAll(PDO::FETCH_ASSOC);
}
?><!DOCTYPE html>
<html lang="en">
<head>
...
<div class="dashboard-container">
    ...
    <form action="edit_course.php?id=<?php echo $courseID; ?>" method="POST" class="slide-up">
    <section class="register-section" >
        <div class="register-container">
       
            <div class="form-container">
                <div><h1 align="center" id="cs">Modify Course</h1></div>
                <br/>
                ...
                <input type="text" name="title" id="title"  class="input-field" placeholder="Title" value="<?php echo $courseData['Title']; ?>" required>
                </div>
                <div class="input-box">
                    
                <input type="text" name="description" id="description"  class="input-field" placeholder="Description"  required<?php echo $courseData['Description']; ?>>
                </div>
                
                <div class="input-box">
                    
                <input type="text" name="keywords" id="keywords" class="input-field"  placeholder="keywords" value="<?php echo $courseData['Keywords']; ?>" required>
                </div> 
                <div class="input-box">
                <label for="professor">Professor:</label>
    <select name="professor" id="professor" required>
        <?php foreach ($professorsData as $professor) : ?>
            <option value="<?php echo $professor['ProfessorID']; ?>" <?php if ($professor['ProfessorID'] === $courseData['ProfessorID']) echo 'selected'; ?>>
                <?php echo $professor['FirstName'] . ' ' . $professor['LastName']; ?>
            </option>
        <?php endforeach; ?>
    </select>
                </div>
        ...
        <?php if (empty($_POST['title'])) : ?>
            <span class="error">Course title is required.</span>
        <?php endif; ?>
        
        <?php if (empty($_POST['description'])) : ?>
            <span class="error">Course description is required.</span>
        <?php endif; ?>
        
        <?php if (empty($_POST['keywords'])) : ?>
            <span class="error">Course keywords are required.</span>
        <?php endif; ?>
        
        <?php if (empty($_POST['professor'])) : ?>
            <span class="error">Course professor is required.</span>
        <?php endif; ?>
    <?php endif; ?>
                <button class="submit" type="submit" value="Edit Course">Save</button>
